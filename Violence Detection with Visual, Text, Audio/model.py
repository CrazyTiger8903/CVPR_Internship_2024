...
class Model(nn.Module):
    def __init__(self, args):
        ...
        if self.fusion == 'concat':
            ...
            self.fc_t = nn.Linear(args.emb_dim, args.hidden_dim)
            self.fc_a = nn.Linear(args.audio_dim, args.hidden_dim)
            self.attention_v = nn.Linear(args.hidden_dim, 1)
            self.attention_t = nn.Linear(args.hidden_dim, 1)
            self.attention_a = nn.Linear(args.hidden_dim, 1)
            self.Aggregate = Aggregate(len_feature=args.hidden_dim)
            self.fc1 = nn.Linear(args.hidden_dim, 512)
    ...
    def forward(self, inputs, text, audio):
        ...
        if self.fusion == 'concat':
            ...
            out_v = self.fc_v(out)
            out_t = self.fc_t(out2)
            out_a = self.fc_a(out3)
            
            out = torch.cat((out_v, out_t, out_a), -1)
        elif self.fusion == 'attention':
            out_v = self.fc_v(out)
            out_t = self.fc_t(out2)
            out_a = self.fc_a(out3)
            
            att_v = torch.softmax(self.attention_v(out_v), dim=1)
            att_t = torch.softmax(self.attention_t(out_t), dim=1)
            att_a = torch.softmax(self.attention_a(out_a), dim=1)
            
            out = out_v * att_v + out_t * att_t + out_a * att_a
        elif self.fusion == 'detour':
            out = out.permute(0, 2, 1)  # for conv1d
            out = self.relu(self.conv1d1(out))
            out = self.drop_out(out)
            out = self.relu(self.conv1d2(out))
            out = self.drop_out(out)
            out = out.permute(0, 2, 1)  # back to b*t*c
            
            out2 = out2.permute(0, 2, 1)  # for conv1d
            out2 = self.relu(self.conv1d3(out2))
            out2 = self.drop_out(out2)
            out2 = self.relu(self.conv1d2(out2))
            out2 = self.drop_out(out2)
            out2 = out2.permute(0, 2, 1)  # back to b*t*c
            
            out = torch.cat((out, out2, out3), -1)  # Concatenate visual, text, and audio features
        #################################################
